import React, { useState } from 'react';
import { GiHamburgerMenu } from 'react-icons/gi';
import { MdOutlineRestaurantMenu } from 'react-icons/md';
import images from '../../constants/images';
import './Navbar.css';

const Navbar = () => {
  const [toggleMenu, setToggleMenu] = useState(false);
  return (
    <nav className="app__navbar">
      <div className="app__navbar-logo">
        <p className="app__navbar-logo-text">ENORM EVENTI</p>
      </div>
      <ul className="app__navbar-links">
        <li className="p__opensans"><a href="#home">Accueil</a></li>
        <li className="p__opensans"><a href="#about">À propos</a></li>
        <li className="p__opensans"><a href="#menu">Services</a></li>
        <li className="p__opensans"><a href="#awards">Distinctions</a></li>
        <li className="p__opensans"><a href="#contact">Contact</a></li>
      </ul>
      <div className="app__navbar-login">
        <a href="#contact" className="p__opensans">Se connecter</a>
        <div />
        <a href="#contact" className="p__opensans">Réserver</a>
      </div>
      <div className="app__navbar-smallscreen">
        <GiHamburgerMenu color="#dcca87" fontSize={27} onClick={() => setToggleMenu(true)} />
        {toggleMenu && (
          <div className="app__navbar-smallscreen_overlay flex__center slide-bottom">
            <MdOutlineRestaurantMenu fontSize={27} className="overlay__close" onClick={() => setToggleMenu(false)} />
            <ul className="app__navbar-smallscreen_links">
              <li><a href="#home" onClick={() => setToggleMenu(false)}>Accueil</a></li>
              <li><a href="#about" onClick={() => setToggleMenu(false)}>À propos</a></li>
              <li><a href="#menu" onClick={() => setToggleMenu(false)}>Services</a></li>
              <li><a href="#awards" onClick={() => setToggleMenu(false)}>Distinctions</a></li>
              <li><a href="#contact" onClick={() => setToggleMenu(false)}>Contact</a></li>
            </ul>
          </div>
        )}
      </div>
    </nav>
  );
};

export default Navbar;